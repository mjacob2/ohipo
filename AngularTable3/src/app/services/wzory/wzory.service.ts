...
export class Calculate {
  ...
  static AssignCurrentWIBORtoOffers(
    offerWibor: string,
    currentWibor3M: number,
    currentWibor6M: number
  ) {
    if (offerWibor === "3M") {
  ...
  static FirstEqualInstallment(
    ...
    if (isOfferFixedRate === "nie") {
      return Calculate.rata(
        creditAmountOfOffer,
        offerWibor + offerMargin,
        creditLengthInYears
      );
    } else {
      return Calculate.rata(
  ...
  static ApartmentInsuranceMonthly(
    apartmentInsuranceCalculatedFrom,
    apartmentInsuranceRatePerYear: number,
    propertyValue,
    creditAmount
  ) {
  ...
  static TotalAppartmentInsurance(
    apartmentInsuranceCalculatedFrom: string,
    apartmentInsuranceMonthly: number,
  ...
  */
   ...
   * P = kwota kredytu
   */
  static odsetkiZaplaconeWcalymOkresie(
    rata: number,
    liczbaLat: number,
    kwotaKredytu: number
  ) {
  ...
  static odsetkiZaplaconeNaKoniecNokresu(
    rata: number,
    liczbaLat: number,
    kwotaKredytu: number,
    r: number
    ...
    r = r / 1200;
    return (
      (kwotaKredytu * r - rata) * (((1 + r) ** (liczbaLat * 12) - 1) / r) +
      rata * liczbaLat * 12
    );
  }
  ...
  static sumOfInterest(
    ...
    propertyValue,
    wiborStawka,
    oprocStaleMarzaPotem,
    oprocStaleIleLat
  ) {
    //**Oblicz sumę ODSETEK RATY RÓWNE */
    if (oprocStale === "nie")
      return Calculate.odsetkiZaplaconeWcalymOkresie(
        rata,
        creditLengthInYears,
        kwotaKredytuOferty
      );
    ...
    if (oprocStale === "jakPEKAO") {
      ...
      for (i = 0; i <= creditLengthInYears * 12; i++) {
        ...
        if (zostaloDoSplaty < propertyValue * 0.8) {
          break;
        }
      }
      ileRazy = i;
      //policz sumę odsetek w okresie kiedy LTV jest > 80%
      let odsetkiZaplaconeAA = Calculate.odsetkiZaplaconeNaKoniecNokresu(
        rata,
        ileRazy / 12,
        kwotaKredytuOferty,
        marza + wiborStawka
      ...
      let rataPotem2 = Calculate.rata(
        kwotaKredytuOferty,
        wiborStawka + oprocStaleMarzaPotem,
        creditLengthInYears
      );

      //Oblicz odsetki zapłacone w całym okresie wg raty potem
      let odsetkiZaplaconeBB = Calculate.odsetkiZaplaconeWcalymOkresie(
        rataPotem2,
        creditLengthInYears,
        kwotaKredytuOferty
      );
      ...
      let odsetkiZaplaconeCC = Calculate.odsetkiZaplaconeNaKoniecNokresu(
        rataPotem2,
        ileRazy / 12,
        kwotaKredytuOferty,
        oprocStaleMarzaPotem + wiborStawka
      );
    ...
    if (oprocStale === "tak") {
      //jesli oprocentowanie stałe TAK

      //policz sumę odsetek w okresie ze stałą stopą
      let odsetkiZaplaconeA = Calculate.odsetkiZaplaconeNaKoniecNokresu(
        rata,
        oprocStaleIleLat,
        kwotaKredytuOferty,
        marza
      ...
      let odsetkiZaplaconeB = Calculate.odsetkiZaplaconeWcalymOkresie(
        rataPotem,
        creditLengthInYears,
        kwotaKredytuOferty
      );
      ...
      let odsetkiZaplaconeC = Calculate.odsetkiZaplaconeNaKoniecNokresu(
        rataPotem,
        oprocStaleIleLat,
        kwotaKredytuOferty,
        oprocStaleMarzaPotem + wiborStawka
  ...
  static totalCostOfLifeInsurance(
    ubezpZycieIleLat,
    creditLengthInYears,
    kwotaKredytuOferty,
    ubezpZycieOdKtoregoMiesiaca,
    rata,
    marza,
    WIBORstawka,
    ubezpZycieStawkaMiesieczna
  ) {
    /** CAŁKOWITY KOSZT UBEZPIECZENIA OD SALDA KREDYTU MALEJACEGO CO ROKU */

    if (ubezpZycieIleLat === 999) {
      // jesi ubezpieczenie jest pobierane przez cały okres kredytu
    ...
    if (ubezpZycieIleLat > 1) {
      //licz tylko jeśli oferta zawiera ubezpieczenie na życie
      ...
      skladkaCalkowita =
        (suma / liczbaLat) *
        ((ubezpZycieStawkaMiesieczna / 100) * 12) *
        liczbaLat;
    }
